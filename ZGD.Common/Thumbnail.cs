...
namespace ZGD.Common
{
    ...
    public class Thumbnail
    {
        ...
        /// 创建缩略图
        /// </summary>
        /// <param name="imgPath">源图片地址</param>
        /// <param name="width">缩略图宽度</param>
        /// <param name="height">缩略图高度</param>
        /// <param name="mode">生成缩略图的方式</param>    
        /// <returns></returns>
        public static string CreateThumbImg(string imgPath, int width, int height, string mode)
        {
            //缩略图生产
            if (!string.IsNullOrWhiteSpace(imgPath))
            {
                string _fileExt = imgPath.Substring(imgPath.LastIndexOf(".") + 1);
                //按日期归类保存
                string _datePath = DateTime.Now.ToString("yyyyMMdd") + "/";
                //获得要保存的文件路径
                string serverFile = "/UpLoadFiles/thum/" + _datePath;
                //物理完整路径                    
                string toFileFullPath = HttpContext.Current.Server.MapPath("~" + serverFile);
                //检查是否有该路径没有就创建
                if (!Directory.Exists(toFileFullPath))
                {
                    Directory.CreateDirectory(toFileFullPath);
                }
                string _fileName = serverFile + Guid.NewGuid().ToString() + "." + _fileExt;
                ZGD.Common.Thumbnail.MakeThumbnail("~" + imgPath, "~" + _fileName, width, height, mode);
                return _fileName;
            }
            return "";
        }

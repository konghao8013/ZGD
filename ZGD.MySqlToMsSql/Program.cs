...
namespace ZGD.MySqlToMsSql
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                return ExSql(my_Data_news, my_Data_news_col, my_Data_news_img);
            });
            Task.WaitAll();
            Console.WriteLine("全部完成（" + task.Result + "）\n ");
            Console.ReadLine();
        }

        public static int ExSql(List<article> my_Data_news, List<articleCol> my_Data_news_col, List<articleImage> my_Data_news_img)
        {
            int count = 0;

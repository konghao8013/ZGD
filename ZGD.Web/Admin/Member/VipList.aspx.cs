...
{
    public partial class VipList : ZGD.BasePage.ManagePage
    {
        ...
        {
            string strWhere = "";
            if (Session["strWhereProduct_VipCard"] != null && Session["strWhereProduct_VipCard"].ToString() != "")
            {
                strWhere = Session["strWhereProduct_VipCard"].ToString();
            }
        ...
        protected void ddlProperty_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            switch (SupplierName)
            {
                case "IsBind":
                    ...
                    if (strsql != "")
                    {
                        Session["strWhereProduct_VipCard"] = " (1=1) " + strsql;
                    }
                    else
                    {
                        Session["strWhereProduct_VipCard"] = "";
                    }
                    break;
                case "IsUnBind":
                    if (SupplierName != "")
                    {
                        strsql += (" and (OpenId is NULL or OpenId = '')");
                    }
                    if (strsql != "")
                    {
                        Session["strWhereProduct_VipCard"] = " (1=1) " + strsql;
        ...
        protected void btnSelect_Click(object sender, EventArgs e)
        {
            ...
            if (SupplierName != "")
            {
                strsql += "and (CardNo like '%" + SupplierName + "%' or OpenId like '%" + SupplierName + "%')";
            }
            if (strsql != "")
            {
                Session["strWhereProduct_VipCard"] = " (1=1) " + strsql;

...
namespace ZGD.Web.Admin.Des
{
    public partial class List : ZGD.BasePage.ManagePage
    {
        ...
        {
            string strWhere = "";
            if (Session["strWhereProduct_Des"] != null && Session["strWhereProduct_Des"].ToString() != "")
            {
                strWhere = Session["strWhereProduct_Des"].ToString();
            }
        ...
        protected void lbtnDel_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    ...
                    //保存日志
                    SaveLogs("[设计师模块]删除设计师：" + model.Title);
                    //删除记录
                    bll.Delete(id);
                    imgBll.DeleteByDesignerId(id);
                }
            }
            JscriptMsg("删除成功");
            RptBind();
        ...
        protected void btnSelect_Click(object sender, EventArgs e)
        {
            ...
            if (SupplierName != "")
            {
                strsql += "and Name like '%" + SupplierName + "%'";
            }
            if (strsql != "")
            {
                Session["strWhereProduct_Des"] = " (1=1) " + strsql;
            }
            else
            {
                Session["strWhereProduct_Des"] = "";
            }

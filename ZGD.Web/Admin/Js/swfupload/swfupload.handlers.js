$(function () {
    ...
    $.fn.InitsSWFUpload = function (p) {
        p = $.extend({}, $.swfUpLoadDefaults.property, p || {});
        //创建上传按钮
        var parentObj = $(this);
        var parentBtnId = "upload_span_" + Math.floor(Math.random() * 1000 + 1);
        parentObj.append('<div class="upload-btn" style=\'border:0px\'><span id="' + parentBtnId + '"></span></div>');
        //初始化属性
        var btnAction = SWFUpload.BUTTON_ACTION.SELECT_FILES; //多文件上传
        ...
        var swfu = new SWFUpload({
            ...
            button_width: p.btnwidth, //上传按钮的宽度
            button_height: p.btnheight, //上传按钮的高度
            button_text: '<span class="btnText">上传</span>', //上传按钮的文字
            button_text_style: ".btnText{font-family:Microsoft YaHei;font-size:12px;line-height:38px;color:#333333;text-align:left;}", //按钮样式
            button_window_mode: SWFUpload.WINDOW_MODE.TRANSPARENT, //背景透明
            button_action: btnAction, //单文件或多文件上传
            button_cursor: SWFUpload.CURSOR.HAND, //指针手形
            flash_url: p.flashurl, //Flash路径
    ...
    try {
        ...
        if (jsonstr.status == '1') {
            ...
            if (this.customSettings.button_action == SWFUpload.BUTTON_ACTION.SELECT_FILE) {
                ...
                $(this.customSettings.upload_target).parents(".w_cover").find(".cover").find("img").attr("src", jsonstr.path);
                $(this.customSettings.upload_target).parents(".w_cover").find(".cover").show();

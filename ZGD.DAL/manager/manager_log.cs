...
namespace ZGD.DAL
{
    ...
    public partial class manager_log
    {
        ...
        {
            return DbHelperSQL.GetMaxID("Id", databaseprefix + "manager_log");
        }
        /// <summary>
        /// 是否存在该记录
        /// </summary>
        ...
        public int Delete(int dayCount)
        {
            ...
            strSql.Append(" where DATEDIFF(day, add_time, date()) > " + dayCount);
        ...
        /// 分页获取数据列表
        /// </summary>
        public DataSet GetList(int pIdx, int pageSize, string strWhere, string sortName, out int rowCount)
        {
            int pageCount = 0;
            rowCount = 0;
            Pager pager = new Pager();
            pager.tableName = databaseprefix + "manager_log";
            pager.fieldsName = " * ";
            pager.sqlWhere = strWhere;
            if (!string.IsNullOrEmpty(sortName))
                pager.orderField = sortName;
            else
                pager.orderField = " add_time desc";
